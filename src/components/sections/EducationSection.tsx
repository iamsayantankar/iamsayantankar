    ...
    <section id="education" className="py-14 sm:py-20">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          <div className="absolute left-4 sm:left-8 top-0 bottom-0 w-px bg-gradient-to-b from-green-500 via-emerald-500 to-transparent" />
          <div className="space-y-6 sm:space-y-8">
            {data.map((edu, i) => (
              <motion.div key={edu._id} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }} className="relative pl-12 sm:pl-20">
                <div className="absolute left-[2px] sm:left-[18px] top-5 sm:top-6 w-7 h-7 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 border-4 border-white dark:border-gray-950 shadow-md flex items-center justify-center z-10">
                  <HiAcademicCap className="w-3.5 h-3.5 text-white" />
                </div>
                <div className="p-4 sm:p-6 rounded-2xl bg-white dark:bg-white/[0.04] border border-gray-200/50 dark:border-white/[0.06]">
