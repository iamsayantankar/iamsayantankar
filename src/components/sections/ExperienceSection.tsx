...
export default function ExperienceSection({ data }: { data: Experience[] }) {
  return (
    <section id="experience" className="py-14 sm:py-20 bg-gray-50/50 dark:bg-gray-900/30">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading title="Work Experience" subtitle="My professional journey" />
        <div className="relative">
          <div className="absolute left-4 sm:left-8 top-0 bottom-0 w-px bg-gradient-to-b from-primary-500 via-purple-500 to-transparent" />
          <div className="space-y-6 sm:space-y-8">
            {data.map((exp, i) => (
              <motion.div key={exp._id} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }} className="relative pl-12 sm:pl-20">
                <div className="absolute left-2 sm:left-6 top-5 sm:top-6 w-5 h-5 rounded-full bg-gradient-to-br from-primary-500 to-purple-600 border-4 border-white dark:border-gray-950 shadow-md z-10" />
                <div className="p-4 sm:p-6 rounded-2xl bg-white dark:bg-white/[0.04] border border-gray-200/50 dark:border-white/[0.06] hover:shadow-lg transition-shadow">
