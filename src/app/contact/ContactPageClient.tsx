    ...
    <PageTransition>
      <section className="pt-24 sm:pt-32 pb-14 sm:pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-16"><h1 className="text-3xl sm:text-5xl font-bold gradient-text mb-3 sm:mb-4">Contact Me</h1><p className="text-gray-500 dark:text-gray-400 text-sm sm:text-base">Let&apos;s build something amazing together</p></div>
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-10">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }}>
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-6 sm:mb-8 text-sm sm:text-base">Feel free to reach out for collaborations or just a friendly hello.</p>
              <div className="space-y-4 mb-6 sm:mb-8">
                {[{ icon: HiEnvelope, l: "Email", v: "hello@sayantankar.dev" }, { icon: HiPhone, l: "Phone", v: "+91 12345 67890" }, { icon: HiMapPin, l: "Location", v: "Kolkata, India" }].map(item => (
                  <div key={item.l} className="flex items-center gap-4"><div className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-primary-50 dark:bg-primary-500/10 flex items-center justify-center text-primary-500 shrink-0"><item.icon className="w-5 h-5" /></div><div className="min-w-0 flex-1"><p className="text-sm text-gray-500">{item.l}</p><p className="font-medium text-gray-900 dark:text-white break-all">{item.v}</p></div></div>
                ))}
              </div>
              <div className="flex flex-wrap gap-3">{[{ icon: FaGithub, h: "https://github.com" }, { icon: FaLinkedin, h: "https://linkedin.com" }, { icon: FaTwitter, h: "https://twitter.com" }].map((s, i) => (<a key={i} href={s.h} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-lg bg-gray-100 dark:bg-white/5 flex items-center justify-center text-gray-600 dark:text-gray-400 hover:bg-primary-500 hover:text-white transition-all"><s.icon className="w-5 h-5" /></a>))}</div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }}>
              <form onSubmit={handleSubmit} className="p-5 sm:p-8 rounded-2xl bg-white dark:bg-white/[0.04] border border-gray-200/50 dark:border-white/[0.06] shadow-lg">
