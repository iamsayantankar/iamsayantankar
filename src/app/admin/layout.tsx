...
function AdminLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-950 flex">
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 max-w-[80vw] bg-gray-900 border-r border-gray-800 transform transition-transform lg:translate-x-0 flex flex-col ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center justify-between p-4 border-b border-gray-800 shrink-0">
          ...
          <button onClick={() => setSidebarOpen(false)} aria-label="Close menu" className="lg:hidden text-gray-400 p-1"><HiXMark className="w-6 h-6" /></button>
        </div>
        ...
        <div className="p-3 border-t border-gray-800 shrink-0">
          <button onClick={() => signOut({ callbackUrl: "/admin/login" })} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm text-red-400 hover:bg-red-500/10 transition-colors">
            <HiArrowRightOnRectangle className="w-5 h-5 shrink-0" /> Sign Out
          </button>
        </div>
      </aside>
      ...
      <div className="flex-1 lg:ml-64 min-w-0">
        <header className="sticky top-0 z-40 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-gray-200 dark:border-gray-800 px-3 sm:px-4 py-3 flex items-center gap-3 sm:gap-4">
          <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5 shrink-0" aria-label="Open menu"><HiBars3 className="w-6 h-6 text-gray-600 dark:text-gray-300" /></button>
          <h1 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white truncate">{navItems.find(i => i.href === pathname)?.label || "Admin"}</h1>
        </header>
