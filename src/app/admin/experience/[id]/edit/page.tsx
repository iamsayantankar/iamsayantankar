...
export default function EditExperience() {
  ...
  useEffect(() => {
    fetch(`/api/experience/${params.id}`).then(r => r.json()).then(d => {
      setForm({
        company: d.company || "", position: d.position || "", duration: d.duration || "",
        ...
        current: d.current || false, description: d.description || "",
        technologies: (d.technologies || []).join(", "), order: d.order || 0,
      });
      setFetching(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/experience/${params.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      if (res.ok) { toast.success("Experience updated!"); router.push("/admin/experience"); }
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-5 sm:mb-6">
        <Link href="/admin/experience" aria-label="Back" className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 shrink-0"><HiArrowLeft className="w-5 h-5 text-gray-500" /></Link>
        <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white truncate">Edit Experience</h2>
      </div>
      <form onSubmit={handleSubmit} className="max-w-3xl space-y-5">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Duration</label><input type="text" value={form.duration} onChange={e => setForm({...form, duration: e.target.value})} className={cls} /></div>
        ...
        <div className="flex flex-wrap gap-3 pt-2">
          ...
          <Link href="/admin/experience" className="px-5 sm:px-6 py-2.5 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">Cancel</Link>
        </div>
      </form>
    </div>
  );
