    ...
    <PageTransition>
      <section className="pt-24 sm:pt-32 pb-14 sm:pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8 sm:mb-12"><h1 className="text-3xl sm:text-5xl font-bold gradient-text mb-3 sm:mb-4">My Projects</h1><p className="text-gray-500 dark:text-gray-400 text-sm sm:text-base">Explore my portfolio of work</p></div>
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 sm:items-center justify-between mb-6 sm:mb-8">
            ...
            <div className="flex flex-wrap gap-2">{cats.map(c => (<button key={c} onClick={() => setFilter(c)} className={`px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-medium transition-all ${filter === c ? "bg-gradient-to-r from-primary-600 to-purple-600 text-white" : "bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400"}`}>{c}</button>))}</div>
          </div>
          <AnimatePresence mode="wait">
            <motion.div key={filter + search} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {filtered.map((p, i) => (
                <motion.div key={p._id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} whileHover={{ y: -6 }} className="group rounded-2xl bg-white dark:bg-white/[0.04] border border-gray-200/50 dark:border-white/[0.06] overflow-hidden hover:shadow-xl transition-all">
                  <div className={`h-40 sm:h-48 bg-gradient-to-br ${grads[i % grads.length]} flex items-center justify-center relative`}>
                    <span className="text-4xl font-bold text-white/30">{p.title.charAt(0)}</span>
                    <div className="absolute inset-0 bg-black/0 sm:group-hover:bg-black/20 transition-colors flex items-center justify-center gap-3 opacity-100 sm:opacity-0 sm:group-hover:opacity-100">
                  ...
                  <div className="p-4 sm:p-5">
                    ...
                    <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white mb-2">{p.title}</h3>
