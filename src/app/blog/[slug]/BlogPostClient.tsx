...
export default function BlogPostClient({ blog }: { blog: Blog }) {
  return (
    <motion.article initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pt-24 sm:pt-32 pb-14 sm:pb-20">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link href="/blog" className="inline-flex items-center gap-2 text-primary-500 hover:text-primary-600 mb-6 sm:mb-8 text-sm sm:text-base"><HiArrowLeft className="w-4 h-4" /> Back to Blog</Link>
        <div className="mb-6 sm:mb-8">
          <span className="text-sm font-medium text-primary-500 mb-3 block">{blog.category}</span>
          <h1 className="text-2xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-4 break-words">{blog.title}</h1>
        ...
        <div className="prose prose-sm sm:prose-base prose-gray dark:prose-invert max-w-none break-words [&_img]:rounded-lg [&_pre]:overflow-x-auto" dangerouslySetInnerHTML={{ __html: blog.content }} />
      </div>
    </motion.article>
  );
}
