          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {filtered.map((b, i) => (
              <motion.div key={b._id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="group rounded-2xl bg-white dark:bg-white/[0.04] border border-gray-200/50 dark:border-white/[0.06] overflow-hidden hover:shadow-lg transition-all">
                <div className={`h-36 sm:h-40 bg-gradient-to-br ${grads[i % grads.length]} flex items-center justify-center`}><span className="text-5xl font-bold text-white/20">{b.title.charAt(0)}</span></div>
                <div className="p-4 sm:p-5"><span className="text-xs font-medium text-primary-500 mb-2 block">{b.category}</span><h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white mb-2 group-hover:text-primary-500 transition-colors line-clamp-2">{b.title}</h3><p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 mb-4">{b.excerpt || "Click to read more..."}</p>
                  <div className="flex items-center justify-between gap-2 flex-wrap"><span className="text-xs text-gray-400 flex items-center gap-1"><HiCalendar className="w-3.5 h-3.5" />{new Date(b.createdAt).toLocaleDateString()}</span><Link href={`/blog/${b.slug}`} className="text-primary-500 text-sm font-medium flex items-center gap-1 hover:gap-2 transition-all">Read More <HiArrowRight className="w-4 h-4" /></Link></div>
                </div>
              </motion.div>
            ))}
          </div>
